...
function renderTable() {
    ...
    document.querySelectorAll('.moveBtn').forEach(button => {
        button.addEventListener('click', function() {
            ...
            const boardIdx = this.closest('tr').querySelector('input[type="checkbox"]').getAttribute('data-board-idx');


            console.log(boardIdx);
            
            if (boardIdx) {
            // board_idx 값을 localStorage에 저장
            localStorage.setItem('board_idx', boardIdx);

                // 게시판 ID를 URL의 쿼리 파라미터로 추가해서 postList.html로 이동
                window.location.href = `postList.html?board_idx=${boardIdx}`;
            }
