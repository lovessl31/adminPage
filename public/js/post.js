...
function renderDetail() {
  ...
  const storedLikeStatus = localStorage.getItem(
    `post_like_${postDetail.post_idx}`
  );
...
function toggleLike() {
  $.ajax({
    ...
    success: function () {
      ...
      likeBtn.attr(
        "src",
        postDetail.isLiked ? "/images/fillheart.png" : "/images/heart.png"
      );
  ...
  switch (option.ol_type) {
                ...
                <div class="module-container">
                    <div class="module-content" id="module-file">
                        ...
                        <div class="file-contentWrap">
                            ${
                              option.selected_value &&
                              option.selected_value.length > 0
                                ? option.selected_value
                                    .map(
                                      (file) => `
                                 <div class="fileInfo">
                                    ...
                                    <span>(${
                                      file.f_size || "크기 정보 없음"
                                    })</span>
                                </div>
                                <button class="file-down-btn" data-f-path="${
                                  file.f_path
                                }" data-f-name="${file.o_f_name}">
    ...
    case "files":
      ...
      const totalSizeKB = files.reduce(
        (sum, file) => sum + convertToKB(file.f_size),
        0
      );
  ...
  postDetail.options.forEach((option) => {
    ...
    if (option.ol_type === "editor") {
      ...
      const viewer = new toastui.Editor.factory({
        ...
        initialValue: option.selected_value
          ? option.selected_value.value
          : "내용 없음",
        viewer: true,
        customHTMLRenderer: {
    ...
    const commentHtml =
      comment.del_yn === "Y"
        ? `
            ...
            `
        : `
            <div class="cmt_item" data-attachments='${JSON.stringify(
              comment.첨부파일
            )}'>
                ...
                <ul class="cmt_drop" style="display:none;">
                    <li><button class="cmt_modify" data-cm-idx="${
                      comment.cm_idx
                    }" data-u-idx="${comment.user_idx}" data-c-content="${
            comment.cm_content
          }" data-u-name="${comment.user_name}">수정</button></li>
                    <li><button class="cmt_delete" data-cm-idx="${
                      comment.cm_idx
                    }" data-u-idx="${comment.user_idx}">삭제</button></li>
                </ul>
                <div class="cmt_content">
                ...
                <div class="reply_btn">
                    <button data-g-idx="${comment.cm_idx}" data-user-name="${
            comment.user_name
          }" data-depth="${comment.depth}">답글</button>
                </div>
            </div>
        `;
...
function replyOn(buttonElement) {
  ...
  if (existingReplyBox.length) {
    ...
    const placeholderText =
      depth === 1 ? "댓글을 입력해주세요" : `${userName}님께 답글쓰기`;
...
function editComment(buttonElement) {
  ...
  function renderAttachments() {
    const imagesHtml = allFiles
      .filter((fileObj) => {
        return (
          (fileObj.type === "existing" &&
            fileObj.data.f_type.startsWith("image/")) ||
          (fileObj.type === "new" && fileObj.data.type.startsWith("image/"))
        );
      })
      .map((fileObj) => {
        ...
        return `
                <div class="thumb_box" data-file-id="${fileObj.id}">
                    <div class="img_cover">
                        <span class="thumbnail"><img src="${imagePath}" alt="${
          file.o_f_name || file.name
        }" class="thumbnail-img"/></span>
  ...
  editTextarea
    .css("height", "auto")
    .css("height", `${editTextarea[0].scrollHeight}px`);
...
function editReplyComment(buttonElement, rp_content) {
  ...
  replyContent = replyContent.replace(
    /<div class="cmt_imgbox">.*?<\/div>/gs,
    ""
  ); // 이미지 박스 제거
  replyContent = replyContent.replace(
    /<div class="cmt_filebox">.*?<\/div>/gs,
    ""
  ); // 파일 박스 제거
  ...
  function findReply(rpIdx) {
    for (const comment of commentsData) {
      const reply = comment.하위댓글.find(
        (subComment) => subComment.rp_idx === rpIdx
      );
  ...
  console.log(
    "editReplyComment 호출됨:",
    `#edit_reply_${rpIdx}`,
    `#fileInputEditReply_${rpIdx}`,
    `#thumb_box_wrap_edit_reply_${rpIdx}`,
    `#file_cover_edit_reply_${rpIdx}`
  );
  ...
  $(`#submitEditReply_${rpIdx}`).on("click", function () {
    ...
    const updatedContent =
      rpPIdx === 0
        ? textareaContent.replace(tagContent, "")
        : `${tagContent}${textareaContent}`;
...
function initializeFileUpload(
  ...
  $(document).off(
    "click",
    ".cmt_pic_wrap, .cmt_pic_wrap_reply, .cmt_pic_wrap_edit, .cmt_pic_wrap_edit_reply"
  );
  $(document).on(
    "click",
    ".cmt_pic_wrap, .cmt_pic_wrap_reply, .cmt_pic_wrap_edit, .cmt_pic_wrap_edit_reply",
    function () {
      const containerSelector = $(this).closest(
        ".edit_write_wrap, .edit_reply_wrap, #comment_write, #reply_write"
      );
                    ...
                    <div class="cmt_fileList">
                        <div>
                            ...
                            <span class="fine_name">
                                ...
                                <button type="button" class="file_name_text">${
                                  file.name
                                }</button>
                            </span>
...
$(function () {
  ...
  $(document).on("click", ".toggle-file-wrap", function () {
    ...
    toggleIcon.attr(
      "src",
      isExpanded
        ? "/images/attach-dropdown.png"
        : "/images/attach-dropright.png"
    );
  ...
  $(document).on("click", ".reply_btn button", function () {
    ...
    // 저장된 데이터 확인
    console.log(
      "Stored g-idx on #submitReply:",
      $("#submitReply").data("g-idx")
    );
  });
  ...
  $(document).on("click", "#submitReply", function (e) {
    ...
    replyContent = replyContent
      .replace(/<span class="fine_name">.*?<\/span>/g, "")
      .trim();
