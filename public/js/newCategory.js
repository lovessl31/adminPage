...
function fetchBoardData(page = 1) {
  ...
  $.ajax({
    url:
      defaultUrl +
      `/with/board_list?option_type=${optionType}&option_value=${optionValue}&per_page=${itemsPerPage}&page=${currentPage}`,
    method: "GET",
    headers: {
  ...
  boards.forEach((board) => {
    ...
    row.on("click", function (e) {
      ...
      if (e.shiftKey && lastSelectedIndex !== null) {
        ...
        // Ctrl 키 또는 Cmd 키를 눌렀을 때는 다중 선택 가능
...
function updateSelectedRows() {
  ...
  $("#boardTableBody tr.selected").each(function () {
    ...
    const boardIdx = parseInt(
      $(this).find("td[data-board-idx]").data("board-idx"),
      10
    ); // user_idx를 숫자로 변환
    // user_idx 추가
...
$(function () {
  ...
  $("#addCate").on("click", function () {
    const selectedNode = selectedNodeId
      ? $("#tree-container").jstree("get_node", selectedNodeId)
      : null;

    // 게시판 노드일 경우 카테고리 추가 금지
    if (selectedNode && selectedNode.type === "board") {
      // selectedNode가 null이 아닐 때만 type 체크
    ...
    if (!selectedNodeId || selectedNodeId === null) {
      ...
      selectedNodeId.startsWith("cate_") ||
      selectedNodeId.startsWith("sub_")
    ) {
      ...
      if (selectedNodeId.startsWith("cate_")) {
        ...
        // 하위 카테고리일 경우
        parentGIdx =
          selectedNode.original?.g_idx ||
          selectedNode.parents
            .find((id) => id.startsWith("cate_"))
            .replace("cate_", ""); // 상위 카테고리의 g_idx 사용
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        $("#tree-container").jstree(
          "set_id",
          newNode,
          response.data.cate_idx || response.data.sub_cate_idx
        ); // 상위/하위 카테고리 ID 반영
        loadTreeData();
      },
    });
    ...
    if (selectedNodeIds.length > 0) {
      ...
      selectedNodeIds.forEach(function (nodeId) {
        ...
        if (node.type === "board") {
          ...
          // 게시판 데이터를 추출
          const parentNodeId = node.parents.find(
            (id) => id.startsWith("cate_") || id.startsWith("sub_")
          ); // 상위 카테고리 cate_idx 추출
          const parentNode = $("#tree-container").jstree(
            "get_node",
            parentNodeId
          ); // 부모 노드 가져오기
  ...
  function sendEditRequest() {
    if (editingNodeId && isEditing) {
      // 편집 완료 후 텍스트 가져오기
      const editingNode = $("#tree-container").jstree(
        "get_node",
        editingNodeId
      );
      ...
      if (
        typeof p_idx === "string" &&
        (p_idx.startsWith("cate_") || p_idx.startsWith("sub_"))
      ) {
  ...
  $(document).on("mousedown", function (event) {
    ...
    if (
      isEditing &&
      !$(event.target).closest("#confirmNode, #cancleNode").length
    ) {
      // 버튼 상태 변경
      $("#confirmNode").hide();
      $("#cancleNode").hide();
  ...
  function updateTeamNameOnServer(
    nodeId,
    newName,
    p_idx,
    g_idx,
  ...
  $("#tree-container").on("drop", function (e) {
    ...
    try {
      ...
      if (
        !targetNode ||
        (targetNode.type !== "Category" && targetNode.type !== "Sub_Category")
      ) {
      ...
      let categoryBoardrIdxs = targetNode.children
        .map((childId) => {
      ...
      const duplicateUsers = droppedData.filter((board) =>
        categoryBoardrIdxs.includes(board.boardIdx)
      );
      ...
      if (duplicateUsers.length > 0) {
        alert(
          "해당 팀에 이미 추가된 사용자가 있습니다: " +
            duplicateUsers.map((b) => b.boardName).join(", ")
        );
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          droppedData.forEach((board) => {
            const randomId =
              "node_" + Date.now() + "_" + Math.floor(Math.random() * 1000); // 임시 노드 ID 생성
            ...
            console.log(
              `사용자 ${board.boardName}가 성공적으로 추가되었습니다. ID 갱신: ${randomId} -> ${newNodeId}`
            );
          });
  ...
  $("#tree-container")
    ...
    .on("move_node.jstree", function (e, data) {
      ...
      moveNodeTimeout = setTimeout(function () {
        ...
        const draggedUserIds = selectedNodes.map((nodeId) =>
          parseInt(nodeId.split("_")[1], 10)
        ); // 선택된 노드들의 user_idx 추출 (숫자로 변환)
        ...
        const newTeamChildren = $("#tree-container").jstree(
          "get_node",
          newTeamId
        ).children;
        const newTeamNodeIds = newTeamChildren
          .map((childId) => {
            const childNode = $("#tree-container").jstree("get_node", childId);
        ...
        const oldParentNode = $("#tree-container").jstree(
          "get_node",
          oldTeamId
        ); // 이동 전 팀의 부모 노드 가져오기
        const newParentNode = $("#tree-container").jstree(
          "get_node",
          newTeamId
        ); // 이동 후 팀의 부모 노드 가져오기
        ...
        $.ajax({
          ...
          success: function (response) {
            ...
            selectedNodes.forEach((nodeId) => {
              ...
              $("#tree-container").jstree("set_id", nodeId, newNodeId);
              console.log(
                `사용자 ${nodeId}가 성공적으로 이동되었습니다. ID 갱신: ${nodeId} -> ${newNodeId}`
              );
            });
