...
function modifyUser(userIdx = null) {
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      // 승인/미승인 상태 선택
      $(`input[name="modifyStatus"][value="${userDetail.status}"]`).prop(
        "checked",
        true
      );
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (selectedCompanyName === null) {
        selectElement.html(
          '<option value="" disabled selected hidden>회사를 선택해주세요.</option>'
        );
      }
...
$(function () {
  ...
  $("#deleteBtn").on("click", function () {
    const selectedUsers = $('tbody input[type="checkbox"]:checked')
      .map(function () {
        return {
