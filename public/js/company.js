...
function fetchCompanyData(page = 1) {
  ...
  $.ajax({
    url:
      defaultUrl +
      `/with/com_list?option_type=${optionType}&option_value=${optionValue}&per_page=${itemsPerPage}&page=${currentPage}`,
    method: "GET",
    headers: {
...
function renderTable() {
  ...
  companies.forEach(function (company, index) {
    ...
    const brcFile = Array.isArray(company.files)
      ? company.files.find((file) => file.sts_cd === "19")
      : null;
        ...
        <tr>
            <td>
                <div class="d-flex align-items-center justify-content-center">
                    <input type="checkbox" class="form-check-input" data-com-idx="${
                      company.com_idx
                    }" data-c-id="${company.c_id}">
                </div>
            </td>
            ...
            <td class="buttons"><button class="userBtn moveBtn" data-com-idx="${
              company.com_idx
            }" data-c-id="${company.c_id}">이동</button></td>
            <td class="buttons"><button class="categoryBtn moveBtn" data-com-idx="${
              company.com_idx
            }" data-c-id="${company.c_id}">이동</button></td>
            <td class="buttons"><button class="boardBtn moveBtn" data-com-idx="${
              company.com_idx
            }" data-c-id="${company.c_id}">이동</button></td>
            <td class="buttons"><button class="teamBtn moveBtn" data-com-idx="${
              company.com_idx
            }" data-c-id="${company.c_id}" data-c-name="${
      company.c_name
    }">이동</button></td>
            <td class="buttons">${approveButton}</td>
            <td class="buttons center-align">
                <button class="modifyBtn comModify" data-id="${
                  company.com_idx
                }">수정</button>
                <button class="deleteBtn" data-com-idx="${
                  company.com_idx
                }" data-c-id="${company.c_id}">삭제</button>
            </td>
...
function modifyComSave() {
  ...
  $.ajax({
    ...
    error: function (error) {
      console.error(
        "회사 정보 수정 오류:",
        error.responseJSON ? error.responseJSON : error.statusText
      );
...
$(function () {
  ...
  $("#deleteBtn").on("click", function () {
    const selectedCompanies = $('tbody input[type="checkbox"]:checked')
      .map(function () {
        return {
          c_id: $(this).data("c-id"),
