...
function createCategoryElement(category, depth, parentCategory = null) {
  ...
  if (category.sub_category && category.sub_category.length > 0) {
    imgRightArrow.addEventListener("click", (e) => {
      ...
      imgRightArrow.src = isExpanded
        ? "./images/rightArrow.png"
        : "./images/downArrow.png";
      imgFolder.src = isExpanded
        ? "./images/folder.png"
  ...
  if (category.sub_category && category.sub_category.length > 0) {
    ...
    ul.style.display = expandedCategories.has(
      category.cate_idx || category.s_c_idx
    )
      ? "block"
      : "none";
...
function cateGetData() {
  ...
  return axios
    .get("http://safe.withfirst.com:28888/with/cate_list", {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    })
...
function addTopCategory() {
  ...
  const addedCategoryDiv = document.querySelector(
    `div[data-category-id='${newCategory.cate_idx}']`
  );
  if (previouslySelectedDiv) {
    previouslySelectedDiv.classList.remove("selected");
  }
  ...
  // 서버에 새로운 카테고리 추가 요청
  axios
    .post(`http://safe.withfirst.com:28888/with/cate_add`, formData, {
      headers: {
...
function addSubCategory(parentLi, parentCategory, depth) {
  ...
  const addedSubCategoryDiv = document.querySelector(
    `div[data-category-id='${newSubCategory.s_c_idx}']`
  );
  if (previouslySelectedDiv) {
    previouslySelectedDiv.classList.remove("selected");
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    ...
    .addEventListener("click", addTopCategory);
