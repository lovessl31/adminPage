    ...
    options.forEach(option => {
        ...
        switch (ol_type) {
            ...
            case 'dateInput':
                ...
                if (required == 'Y') {
                    $(`#data_input_box_${filesInputCnt}.requireCheckbox`).prop('checked', true); 
                }

                break;

                case 'textArea':
                createTextAreaInput(sort_num, 'update', ol_idx); // 텍스트 영역 필드 생성 시 sort_num 전달
                $(`#textarea_input_box_${textAreanCnt} .attributeName`).val(ol_name); // 속성명 설정
                $(`#textarea_input_box_${textAreanCnt} .sortNum`).val(sort_num); // sort_num 값 설정
				$(`#data_input_box_${textAreanCnt}`).attr('data-gubun', 'update');

				 // view_sts 값이 '2'라면 체크박스를 체크 상태로 설정
				 if (view_sts === '2') {
                    $(`#data_input_box_${textAreanCnt} input[type="checkbox"].hiddenCheckbox`).prop('checked', true);
                }

                if (required == 'Y') {
                    $(`#data_input_box_${textAreanCnt}.requireCheckbox`).prop('checked', true); 
                }

                break;

                case 'editor':
                createEditorInput(sort_num, 'update', ol_idx); // 에디터 필드 생성 시 sort_num 전달
                $(`#editor_input_box_${editorCnt} .attributeName`).val(ol_name); // 속성명 설정
                $(`#editor_input_box_${editorCnt} .sortNum`).val(sort_num); // sort_num 값 설정
				$(`#data_input_box_${editorCnt}`).attr('data-gubun', 'update');

				 // view_sts 값이 '2'라면 체크박스를 체크 상태로 설정
				 if (view_sts === '2') {
                    $(`#data_input_box_${editorCnt} input[type="checkbox"].hiddenCheckbox`).prop('checked', true);
                }

                if (required == 'Y') {
                    $(`#data_input_box_${editorCnt} .requireCheckbox`).prop('checked', true); 
                }
