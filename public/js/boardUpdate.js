...
function boardInfoRender(boardDetail) {
  ...
  // 게시판 유형 설정
  $(`input[name="boardType"][value="${boardDetail.board_type}"]`).prop(
    "checked",
    true
  );

  // 좋아요 설정 (like_set) 설정
  $(`input[name="likeOption"][value="${boardDetail.like_set}"]`).prop(
    "checked",
    true
  );

  // 댓글 설정 (commentSet) 설정
  $(`input[name="cmtOption"][value="${boardDetail.commentSet}"]`).prop(
    "checked",
    true
  );
    ...
    switch (ol_type) {
      case "dropdown":
        ...
        if (!view_sts.admin_list_view) {
          $(
            `#dropdown_box_${dropDownCnt} input[type="checkbox"].hiddenCheckbox`
          ).prop("checked", true);
        }

        if (view_sts.user_list_view) {
          $(
            `#dropdown_box_${dropDownCnt} input[type="checkbox"].userCheckbox`
          ).prop("checked", true);
        }

        if (required === "Y") {
          $(`#dropdown_box_${dropDownCnt} .requireCheckbox`).prop(
            "checked",
            true
          );
        }
      ...
      case "dataInput":
        ...
        if (!view_sts.admin_list_view) {
          $(
            `#data_input_box_${dataInputCnt} input[type="checkbox"].hiddenCheckbox`
          ).prop("checked", true);
        }
        if (view_sts.user_list_view) {
          $(
            `#data_input_box_${dataInputCnt} input[type="checkbox"].userCheckbox`
          ).prop("checked", true);
        }

        if (required == "Y") {
          $(`#data_input_box_${dataInputCnt} .requireCheckbox`).prop(
            "checked",
            true
          );
        }
      ...
      case "dateInput":
        ...
        if (!view_sts.admin_list_view) {
          $(
            `#date_input_box_${dateInputCnt} input[type="checkbox"].hiddenCheckbox`
          ).prop("checked", true);
        }

        if (view_sts.user_list_view) {
          $(
            `#date_input_box_${dateInputCnt} input[type="checkbox"].userCheckbox`
          ).prop("checked", true);
        }

        if (required == "Y") {
          $(`#date_input_box_${dateInputCnt} .requireCheckbox`).prop(
            "checked",
            true
          );
        }
      ...
      case "file_video":
        ...
        if (ol_type === "file") {
          $(`#file_input_box_${fileInputCnt} .file-btn`).removeClass(
            "selectedBtn"
          );
          $(`#file_input_box_${fileInputCnt} .file-btn:first`).addClass(
            "selectedBtn"
          ); // 전체 버튼 선택
          ...
          $(`#file_input_box_${fileInputCnt} .file-btn:eq(1)`).addClass(
            "selectedBtn"
          ); // 이미지 버튼 선택
          ...
          $(`#file_input_box_${fileInputCnt} .file-btn:eq(2)`).addClass(
            "selectedBtn"
          ); // 비디오 버튼 선택
          $(`#file_input_box_${fileInputCnt} .thumbImgWrapper`).hide(); // 대표 이미지 설정 숨기기
        }
        ...
        if (!view_sts.admin_list_view) {
          // admin_list_view가 false일 때 체크박스 선택
          $(
            `#file_input_box_${fileInputCnt} input[type="checkbox"].hiddenCheckbox`
          ).prop("checked", true);
        }

        if (view_sts.user_list_view) {
          $(
            `#file_input_box_${fileInputCnt} input[type="checkbox"].userCheckbox`
          ).prop("checked", true); // 사용자 목록에 보임 설정
        }

        // 대표 이미지 설정 라디오 버튼 처리 (file_img 타입에서만 적용)
        if (ol_type === "file_img") {
          const isThumbnail = view_sts.thumbnail || false;
          $(`#file_input_box_${fileInputCnt} input[name="thumbImg"]`).prop(
            "checked",
            isThumbnail
          );
        }
        ...
        if (required === "Y") {
          $(`#file_input_box_${fileInputCnt} .requireCheckbox`).prop(
            "checked",
            true
          );
        }
      ...
      case "files":
        ...
        if (!view_sts.admin_list_view) {
          $(
            `#files_input_box_${filesInputCnt} input[type="checkbox"].hiddenCheckbox`
          ).prop("checked", true);
        }
        if (view_sts.user_list_view) {
          $(
            `#files_input_box_${filesInputCnt} input[type="checkbox"].userCheckbox`
          ).prop("checked", true);
        }

        // required 값이 'Y'라면 requireCheckbox 체크박스를 체크 상태로 설정
        if (required === "Y") {
          $(`#files_input_box_${filesInputCnt} .requireCheckbox`).prop(
            "checked",
            true
          );
        }
      ...
      case "textArea":
        ...
        if (!view_sts.admin_list_view) {
          $(
            `#textarea_input_box_${textAreanCnt} input[type="checkbox"].hiddenCheckbox`
          ).prop("checked", true);
        }
        if (view_sts.user_list_view) {
          $(
            `#textarea_input_box_${textAreanCnt} input[type="checkbox"].userCheckbox`
          ).prop("checked", true);
        }

        if (required === "Y") {
          $(`#textarea_input_box_${textAreanCnt} .requireCheckbox`).prop(
            "checked",
            true
          );
        }
      ...
      case "editor":
        ...
        if (!view_sts.admin_list_view) {
          $(
            `#editor_input_box_${editorCnt} input[type="checkbox"].hiddenCheckbox`
          ).prop("checked", true);
        }

        if (view_sts.user_list_view) {
          $(
            `#editor_input_box_${editorCnt} input[type="checkbox"].userCheckbox`
          ).prop("checked", true);
        }

        if (required === "Y") {
          $(`#editor_input_box_${editorCnt} .requireCheckbox`).prop(
            "checked",
            true
          );
        }
...
function updateSortNumbers() {
  $("#optionsBody")
    ...
    .each(function (index) {
      $(this)
        .find(".sortNum")
        .val(index + 1); // 각 모듈의 순번을 1부터 다시 재정렬
    });
...
function createDropDown(sort_num = null, gubun = "create", ol_idx = "") {
  dropDownCnt++;
  let html = createDropdownContent(
    dropDownCnt,
    gubun,
    sort_num || ++currentMaxSortNum,
    ol_idx,
    ""
  ); // 동적으로 gubun 값 설정
...
function createDropdownContent(dropDownCnt, gubun, sort_num, ol_idx, name) {
  ...
  html += `    <button class="createCancle" onclick="javascript:removeOptionBox('dropdown_box_${dropDownCnt}', ${
    ol_idx ? ol_idx : null
  }, '${gubun}');" data-action="cancel">취소</button>`;
  html += `</div></div>`;
...
function createDataInput(sort_num = null, gubun = "create", ol_idx = "") {
  dataInputCnt++;
  let html = createDataInputContent(
    dataInputCnt,
    gubun,
    sort_num || ++currentMaxSortNum,
    ol_idx,
...
function createDataInputContent(dataInputCnt, gubun, sort_num, ol_idx, name) {
  ...
  container.innerHTML = `
        ...
        <div class="createBtn">
            <button class="createCancle" onclick="removeOptionBox('data_input_box_${dataInputCnt}', ${
    ol_idx ? ol_idx : null
  }, '${gubun}');" data-action="cancel">취소</button>
        </div>
    `;
...
function createDateInput(sort_num = null, gubun = "create", ol_idx = "") {
  dateInputCnt++;
  let html = createDateInputContent(
    dateInputCnt,
    gubun,
    sort_num || ++currentMaxSortNum,
    ol_idx,
...
function createDateInputContent(dateInputCnt, gubun, sort_num, ol_idx, name) {
  ...
  container.innerHTML = `
        ...
        <div class="createBtn">
            <button class="createCancle" onclick="removeOptionBox('date_input_box_${dateInputCnt}', ${
    ol_idx ? ol_idx : null
  }, '${gubun}');" data-action="cancel">취소</button>
        </div>
    `;
...
function createFileInput(sort_num = null, gubun = "create", ol_idx = "") {
  fileInputCnt++;
  let html = createFileInputContent(
    fileInputCnt,
    gubun,
    sort_num || ++currentMaxSortNum,
    ol_idx,
...
function createFileInputContent(fileInputCnt, gubun, sort_num, ol_idx, name) {
  ...
  container.innerHTML = `
        ...
        <div class="createBtn">
            <button class="createCancle" onclick="removeOptionBox('file_input_box_${fileInputCnt}', ${
    ol_idx ? ol_idx : null
  }, '${gubun}');" data-action="cancel">취소</button>
        </div>
    `;
...
function createFilesInput(sort_num = null, gubun = "create", ol_idx = "") {
  ...
  let html = createFilesInputContent(
    ...
    ol_idx,
    ""
  ); // 동적으로 gubun 값 설정  // 새로운 모듈은 gubun 값으로 'create' 전달
  $("#optionsBody").append(html);
  updateSortNumbers(); // 추가 후 순번 업데이트
}
...
function createFilesInputContent(filesInputCnt, gubun, sort_num, ol_idx, name) {
  ...
  container.innerHTML = `
        ...
        <div class="createBtn">
            <button class="createCancle" onclick="removeOptionBox('files_input_box_${filesInputCnt}', ${
    ol_idx ? ol_idx : null
  }, '${gubun}');" data-action="cancel">취소</button>
        </div>
    `;
...
function createTextAreaInputContent(
  ...
  container.innerHTML = `
        ...
        <div class="createBtn">
            <button class="createCancle" onclick="removeOptionBox('textarea_input_box_${textAreanCnt}', ${
    ol_idx ? ol_idx : null
  }, '${gubun}');" data-action="cancel">취소</button>
        </div>
    `;
...
function createEditorInputContent(editorCnt, gubun, sort_num, ol_idx, name) {
  ...
  container.innerHTML = `
        ...
        <div class="createBtn">
            <button class="createCancle" onclick="removeOptionBox('editor_input_box_${dataInputCnt}', ${
    ol_idx ? ol_idx : null
  }, '${gubun}');" data-action="cancel">취소</button>
        </div>
    `;
...
function collectDynamicOptions() {
  ...
  $("#optionsBody")
    ...
    .each(function () {
      ...
      if (type === "file_img") {
        const isThumbnailChecked =
          $this.find('input[type="radio"][name="thumbImg"]:checked').length > 0;
...
$(function () {
  ...
  $("#boardSaveBtn").on("click", function () {
    ...
    if (selectedCategory) {
      // selectedCategory가 존재할 경우에만 값 설정
