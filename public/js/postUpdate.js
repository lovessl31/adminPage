    ...
    switch (option.ol_type) {
      ...
      case "dropdown":
        moduleElement = `
                    ...
                    <h5 class="module_name ${requiredClass}">${
          option.ol_name
        }</h5>
                        <select id="dropdown_${index}">
                            ${option.ol_value
                              .map(
                                (value) =>
                                  `<option value="${value.ol_value}">${value.ol_value}</option>`
                              )
                              .join("")}
                        </select>
                    </div>
    ...
    switch (option.ol_type) {
      ...
      case "files":
        ...
        if (Array.isArray(selectedValue)) {
          ...
          const totalVolumeElement = $(`#multiFileBtn_${index}`)
            .closest(".file_attach_wrap")
            .find(".total_volume em");
          const totalVolumeText = $(`#multiFileBtn_${index}`)
            .closest(".file_attach_wrap")
            .find(".total_volume");
  ...
  state.options.forEach((option, index) => {
    ...
    switch (option.ol_type) {
      case "dropdown":
        ...
        const selectedOption = option.ol_value.find(
          (v) => v.ol_value === selectedDropdownValue
        );
      ...
      case "files":
        ...
        const nonDeletedFiles = fileList.filter(
      ...
      case "file": // 단일 파일 입력 모듈
        ...
        // 기존 파일이 있는지 확인
        const existingSingleFile = singleFile.find(
          (f) => f.ol_idx === option.ol_idx && f.action === "saved"
        );
        ...
        if (
          isRequired &&
          !existingSingleFile &&
          (!singleFileInput || singleFileInput.files.length === 0)
        ) {
          Swal.fire(
            "경고",
            `${option.ol_name} 파일을 첨부해주세요.`,
        ...
        if (singleFileInput && singleFileInput.files.length > 0) {
          ...
          // 기존 파일이 있는지 확인
          const existingFileIndex = singleFile.findIndex(
            (f) => f.ol_idx === option.ol_idx && f.action === "saved"
          );
      ...
      case "file_img": // 이미지 파일만 허용
        ...
        // 기존 이미지 파일 확인
        const existingImgFile = singleFile.find(
          (f) => f.ol_idx === option.ol_idx && f.action === "saved"
        );
        ...
        if (
          isRequired &&
          !existingImgFile &&
          (!imgFileInput || imgFileInput.files.length === 0)
        ) {
          Swal.fire(
            "경고",
            `${option.ol_name} 이미지를 첨부해주세요.`,
            "warning"
          );
          missingRequiredField = true;
          return;
      ...
      case "file_video": // 비디오 파일만 허용
        ...
        // 기존 비디오 파일 확인
        const existingVideoFile = singleFile.find(
          (f) => f.ol_idx === option.ol_idx && f.action === "saved"
        );
        ...
        if (
          isRequired &&
          !existingVideoFile &&
          (!videoFileInput || videoFileInput.files.length === 0)
        ) {
          Swal.fire(
            "경고",
            `${option.ol_name} 비디오 파일을 첨부해주세요.`,
            "warning"
          );
          missingRequiredField = true;
          return;
      ...
      case "checkbox":
        ...
        if (
          ...
          opt_value !== option.selected_value.value
        ) {
...
function initializeMultiFileUpload(
  ...
  const totalVolumeElement = $(fileBtnId)
    .closest(".file_attach_wrap")
    .find(".total_volume em");
  const totalVolumeText = $(fileBtnId)
    .closest(".file_attach_wrap")
    .find(".total_volume");
  ...
  deleteAllBtn.on("click", function () {
    ...
    // 'add' 상태의 파일은 목록에서 제거
    multiFilesInput[olIdx] = multiFilesInput[olIdx].filter(
      (fileObj) => fileObj.action !== "add"
    );
...
function renderFileList(olIdx, fileList, deleteAllBtn) {
  ...
  multiFilesInput[olIdx].forEach((fileObj, index) => {
    if (fileObj.action !== "delete") {
      ...
      const fileRow = $(`<tr>
                ...
                <td class="text-right">${(fileObj.file.size / 1024).toFixed(
                  2
                )}KB</td>
                <td></td>
            </tr>`);
  ...
  deleteAllBtn.toggle(
    multiFilesInput[olIdx].some((fileObj) => fileObj.action !== "delete")
  );
}
...
function updateTotalVolume(olIdx, totalVolumeElement, totalVolumeText) {
  ...
  const totalCount = multiFilesInput[olIdx].filter(
    (fileObj) => fileObj.action !== "delete"
  ).length;

  // 파일 개수와 용량 표시 업데이트
  totalVolumeElement.text(totalCount);
  totalVolumeText.html(
    `첨부파일 <em>${totalCount}</em>개 (${(totalSize / 1024).toFixed(2)}KB)`
  );
}
...
function initializeSingleFileUpload(
  ...
  deleteBtn
    ...
    .on("click", function () {
      ...
      const fileObjIndex = singleFile.findIndex(
        (f) =>
          f.ol_idx === olIdx &&
          (f.action === "add" || f.action === "update" || f.action === "saved")
      );
