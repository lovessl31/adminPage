...
function renderBoardTable() {
          ...
          <tr>
                <td>
                    <div class="d-flex align-items-center justify-content-center">
                        <input type="checkbox" class="form-check-input" data-b-idx="${
                          board.board_idx
                        }" data-b-name="${board.board_name}">
                    </div>
                </td>
                <td style="text-align : left;">${board.board_idx}</td>
                <td style="text-align : left;">
                    <p class="moveBoardPage" data-b-type="${
                      board.board_type
                    }" data-b-idx="${board.board_idx}">${
      board.board_name
    }<img src="./images/link.svg"></p>
                </td>
                <td>${boardType}</td>
                <td>${commentSet}</td>
                <td>${likeSet}</td>
                ...
                <td class="buttons center-align">
                    <button class="modifyBtn" id="boardModify" data-b-idx="${
                      board.board_idx
                    }">수정</button>
                    <button class="deleteBtn" data-b-idx="${
                      board.board_idx
                    }" data-b-name="${board.board_name}">삭제</button>
                    <button class="moveBtn moveBoardPage" data-b-idx="${
                      board.board_idx
                    }" data-b-type="${board.board_type}">이동</button>
                </td>
            </tr>
        `);
  ...
  $("#boardTableBody").on(
    "click",
    ".modifyBtn",
    moveToUpdate("boardUpdate.html")
  );
  $("#boardTableBody").on("click", ".deleteBtn", deleteBoard);
  $("#boardTableBody").on(
    "click",
    ".moveBoardPage",
    moveToPage("postList.html")
  );
}
...
$(function () {
  ...
  $("#deleteBtn").on("click", function () {
    const selectedBoards = $('tbody input[type="checkbox"]:checked')
      .map(function () {
        return {
