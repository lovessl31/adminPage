...
function fetchPostData(page = 1) {
  ...
  $.ajax({
    url:
      defaultUrl +
      `/with/post_list?bidx=${bidx}&option_type=${optionType}&option_value=${optionValue}&per_page=${itemsPerPage}&page=${currentPage}`,
    method: "GET",
    headers: {
  ...
  if (posts.length === 0) {
    ...
    posts.forEach((post) => {
      ...
      uniqueHeaders.forEach((header) => {
        ...
        const textOpt = post.post_opt.opt_text?.find(
          (opt) => opt.ol_name === header
        );
        if (textOpt) {
          if (textOpt.ol_type === "editor") {
            ...
            const textContent =
              tempDiv.textContent.trim().slice(0, 50) +
              (tempDiv.textContent.length > 50 ? "..." : "");
            const image = tempDiv.querySelector("img");
            const imageContent = image
              ? `<img src="${image.src}" alt="썸네일" class="editor_thumbnail">`
              : "";
        ...
        const selOpt = post.post_opt.opt_sel?.find(
          (opt) => opt.ol_name === header
        );
        if (selOpt) {
        ...
        const fileOpt = post.post_opt.post_files?.find(
          (opt) => opt.ol_name === header
        );
        if (fileOpt) {
...
$(function () {
  ...
  $("#deleteBtn").on("click", function () {
    const selectedPosts = $('tbody input[type="checkbox"]:checked')
      .map(function () {
        return {
